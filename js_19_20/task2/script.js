...
var allSkills = _.flattenDeep(skills);
var uniqSkills = _.uniq(allSkills);
var sortedSkills = _.sortBy(uniqSkills);
...
var names = _.map(sortedData, 'name');

console.log('Names, sorted by friends: ', names);


/* 3. Массив всех друзей всех пользователей, не должно быть повторяющихся людей */

var friends = _.map(data, 'friends');
var allFriends = _.flattenDeep(friends);
var friendsName = _.map(allFriends, 'name');
var uniqFriends = _.uniq(friendsName);

console.log('Unique friends name: ', uniqFriends);